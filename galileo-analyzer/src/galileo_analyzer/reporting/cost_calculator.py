...
class CostCalculator:
    ...
    WORKER_COSTS = {
        'Standard': 0.44, 'G.1X': 0.44, 'G.2X': 0.88, 
        'G.4X': 1.76, 'G.8X': 3.52, 'Z.2X': 1.00
    }
    
    USD_TO_BRL = 5.2  # Pode vir de config
    
    @classmethod
    def quick_cost_estimate(cls, job_details: Dict, job_runs: List[Dict]) -> CostEstimate:
        ...
        if job_runs:
            execution_times = [run.get('ExecutionTime', 0) for run in job_runs if run.get('ExecutionTime')]
            if execution_times:
